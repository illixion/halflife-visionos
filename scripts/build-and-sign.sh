...
if [[ "$NO_DEPLOY" == false ]]; then
    ...
    # Resolve by pattern, never by column position: the Hostname column is
    # empty while a device sits in the `connected` state, which shifts every
    # later field left and makes `awk '{print $3}'` return the literal
    # string "connected" instead of the identifier (see ~/bin/build-and-sign
    # for the same fix). Simulator rows are skipped since devicectl only
    # installs to physical devices.
