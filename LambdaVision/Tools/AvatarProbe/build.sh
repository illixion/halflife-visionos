...
#   ./build.sh --shells   also write shell_<weapon>_{idle,reload}.tri here: the
#                         viewmodel's gun with its p_ hull, idle and mid-reload
...
swiftc -O -g -I "$rk" -L "$rk" -lRAVERig \
    ...
    "$app/AvatarRig.swift" "$app/ViewmodelGrip.swift" "$app/ViewmodelShell.swift" \
    "$here/main.swift" "$here/viewmodels.swift" "$here/shells.swift" "$here/stubs.c" \
