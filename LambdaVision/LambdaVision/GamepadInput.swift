...
//  the SwiftUI hierarchy needs
//  .handlesGameControllerEvents(matching: .gamepad), or the system keeps
//  the pad for focus navigation and polled values freeze.
//
...
nonisolated final class GamepadInput {
    ...
    private var configured: GCController?
    ...
    func poll(snapTurn: (Float) -> Void) {
        // Discovery: cheap enough to re-check per frame; GCController state
        // reads are snapshot-based and thread-agnostic.
        let controller = GCController.current ?? GCController.controllers().first
        if let controller, controller !== configured {
            configured = controller
            controller.playerIndex = .index1
            // Register as a persistent consumer so visionOS routes events
            // to us instead of system focus navigation.
            controller.extendedGamepad?.valueChangedHandler = { _, _ in }
            print("[LambdaVision] gamepad connected: \(controller.vendorName ?? "unknown")")
        }
        guard let pad = configured?.extendedGamepad else { return }
    ...
    private func edge(_ pressed: Bool, _ cmd: String) {
        if pressed != held.contains(cmd) {
            if pressed { held.insert(cmd) } else { held.remove(cmd) }
